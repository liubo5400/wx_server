...
public class JwMediaAPI {
	...
	//新增临时素材
	private static String media_upload_url = "https://api.weixin.qq.com/cgi-bin/media/upload?access_token=ACCESS_TOKEN&type=TYPE";
	// 获取临时素材
	private static String media_get_url = "https://api.weixin.qq.com/cgi-bin/media/get?access_token=ACCESS_TOKEN&media_id=MEDIA_ID";
	// 新增永久图文素材
	...
	public static WxUpload uploadMedia(String accessToke,String type,String fileNamePath) throws WexinReqException{
		...
		logger.info("上传多媒体接口返回："+result.toString());
	...
	public static WxDwonload downMedia(String accessToke,String media_id,String filePath) throws WexinReqException{
		...
		logger.info("下载多媒体接口返回："+result.toString());
		Object error = result.get(WeiXinConstant.RETURN_ERROR_INFO_CODE);
	...
	public static void main(String[] args){
		...
		try {
			WxUpload s = JwMediaAPI.uploadMedia(
					"JHj7xrZtCWImeI4VA2tqW_W5rT_15lQs6f9bZ0cQzmA65YRAD5SuetLyFkJ1lhiXnY6XdxviCG1gkAXqdquCOcIt-fo7sYCcvSdx7cIeePq1caIu9Nx9-mnWj5BSLNqqPKDgAJASNG",
					"mp3","D:\\001.mp3");
			System.out.println(s.getMedia_id());
...
//			System.out.println(d.getFileName());
	...
	public static WxMediaResponse uploadMedia(String accesstoken, WxMedia wxMedia) throws WexinReqException {
		WxMediaResponse wxMediaResponse = null;

		if (accesstoken != null) {
			String requestUrl = media_upload_url.replace("ACCESS_TOKEN", accesstoken);
			if (wxMedia.getFileName() != null && wxMedia.getFileName().length() > 0) {
				requestUrl = requestUrl.replace("TYPE",wxMedia.getType());
				File file = new File(wxMedia.getFilePath() + wxMedia.getFileName());
				String contentType = WeiXinReqUtil.getFileContentType(wxMedia.getFileName().substring(wxMedia.getFileName().lastIndexOf(".") + 1));
				JSONObject result = WxstoreUtils.uploadMediaFile(requestUrl, file, contentType);
				System.out.println("微信上传临时媒体返回的结果：" + result.toString());
				if (result.has("errcode")) {
					logger.error("上传临时媒体消息失败！errcode=" + result.getString("errcode") + ",errmsg = " + result.getString("errmsg"));
					throw new WexinReqException("上传临时媒体消息失败！errcode=" + result.getString("errcode") + ",errmsg = " + result.getString("errmsg"));
				} else {
					wxMediaResponse = new WxMediaResponse();
					wxMediaResponse.setMedia_id(result.getString("media_id"));
					wxMediaResponse.setType(result.getString("type"));
					wxMediaResponse.setCreated_at(new Date(result.getLong("created_at") * 1000));
				}
			}
		}
	...
	public static String  getMedia(String accesstoken,String mediaId) throws WexinReqException {

		String mediaUrl = null;
		if (accesstoken != null && StringUtils.isNotBlank(mediaId)) {
			...
			System.out.println("微信获得临时媒体返回的结果：" + result.toString());
			if (result.has("errcode")) {
				logger.error("获得临时媒体消息失败！errcode=" + result.getString("errcode") + ",errmsg = " + result.getString("errmsg"));
				throw new WexinReqException("获得临时媒体消息失败！errcode=" + result.getString("errcode") + ",errmsg = " + result.getString("errmsg"));
			} else {
				mediaUrl = result.getString("video_url");
			}
		}
		return mediaUrl;
	}
...
//		String accessToken = JwTokenAPI.getAccessToken("wx88ca88a932f2f72a","b11eb0f81375d95e8cdd35902b729ea8");
//		WxNews wxNews = queryArticlesByMaterial(accessToken, "image", 0, 20);
//		System.out.println(new Gson().toJson(wxNews));
//	}
